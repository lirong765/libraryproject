...
<script>
    layui.use(['form', 'table'], function () {
        ...
        table.render({
            elem: '#currentTableId',
            url: '<%=basePath%>lendListAll',//查询借阅信息
            toolbar: '#toolbarDemo',
            ...
            cols: [[
                ...
                {
                    templet: '<div><a href="javascript:void(0)" style="color:#0000FF" lay-event="bookInfoEvent">{{d.bookInfo.name}}</a></div>',
                    width: 100,
                    title: '书名'
                },
                {templet: '<div>{{d.readerCard.cardnumber}}</div>', width: 100, title: '借书卡号'},
                {
                    templet: '<div><a href="javascript:void(0)" style="color:#0000FF" lay-event="readerInfoEvent">{{d.readerCard.username}}</div>',
                    width: 130,
                    title: '借阅人'
                },
                {
                    templet: "<div>{{layui.util.toDateString(d.leadDate, 'yyyy-MM-dd HH-mm-ss')}}</div>",
                    width: 200,
                    title: '借阅时间'
                },
                {field: "backDate", width: 200, title: '归还时间'},
                {
                    title: "还书类型", minWidth: 102, templet: function(res) {
                        if (res.type == '0') {
        ...
        table.on('tool(currentTableFilter)', function (obj) {
            ...
            } else if (obj.event === 'readerInfoEvent') {     //读者借阅线
                layer.msg("读者借阅线")
                //获取读者的id
                var rid = data.readerId;
                queryLookBookList("reader", rid);
            }
        });
            ...
            if (obj.event === 'add') {  // 监听添加操作
                ...
                */
                ...
                if (data.length == 0) {//如果没有选中信息
                    ...
                    var bookIds = getCheackBookId(data); //获取选中记录的图书id集合
                    layer.confirm('真的删除行么', function (index) {
                        //调用删除功能
